...
### 5-1. NestJS 구성요소 중 도메인

- kebab-case + 점 구분 **`<도메인>.<역할>.ts`**
- `nest g` 자동 생성 규칙과 동일
- 예: `users.controller.ts`, `spec-parser.service.ts`, `create-user.dto.ts`, `endpoints.module.ts`

### 5-2. TypeScript 타입 파일

- 타입 정의 전용 파일: `<이름>.type.ts`
- 예: `spec.type.ts` (SpecDocument, SpecResult 등)
...
| 파일 (Nest 도메인) | `<도메인>.<역할>.ts` | `project.service.ts` |
| 파일 (ts 타입 파일) | `<이름>.type.ts` | `spec.type.ts` |
